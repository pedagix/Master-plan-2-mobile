...
function maybeGenerateQuestion(capture) {
  ...
  return {
    ...
    question: 'What is the most important missing detail that would unblock this capture?',
    reason: 'This capture looks like it may contain uncertainty, a blocker, or an unclear assumption.',
    questionType: 'clarify-blocker',
    state: 'open',
    createdAt: Date.now(),
    answeredAt: null,
    answerNoteId: null,
...
export default function CapturePage({ api }) {
  ...
  const submit = (e) => {
    ...
    const capture = { id: crypto.randomUUID(), text, projectId: projectId || null, isNewIdea, createdAt: Date.now(), createdWithPromptProfileId: api.data.settings.activePromptProfileId };
    const question = maybeGenerateQuestion(capture);
    api.setData((prev) => ({ ...prev, captures: [capture, ...prev.captures], questions: question ? [question, ...prev.questions] : prev.questions }));
