...
export default function CapturePage({ api }) {
  ...
  const submit = (e) => {
    ...
    const capture = { id: crypto.randomUUID(), text, projectId: selectedProjectId, isNewIdea, rawState: 'unprocessed', analysisState: 'not-analyzed', processedAt: null, archivedRawAt: null, needsReanalysis: false, needsProjectAssignment: isNewIdea, candidateProjectIds: [], processingTags: [], createdAt: now, createdWithPromptProfileId: api.data.settings.activePromptProfileId };
    api.setData((prev) => ({ ...prev, captures: [capture, ...prev.captures], projects: createdProject ? [createdProject, ...prev.projects] : prev.projects.map((p) => p.id === capture.projectId ? { ...p, lastInteractedAt: now, interactionCount: (p.interactionCount || 0) + 1 } : p), settings: { ...prev.settings, lastSelectedProjectId: selectedProjectId } }));
