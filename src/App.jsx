...
function LoginGate() {
  return (
    <div className="stack">
      <h2>Sign in to Master Plan</h2>
      <p>Use Google sign-in to sync your private data with Firebase.</p>
...
export default function App() {
  ...
  useEffect(() => {
    ...
    return listenToAuthState(setUser);
  }, []);
  ...
  useEffect(() => {
    ...
    loadUserData(user.uid)
      .then((remoteData) => {
        if (remoteData?.projects?.length || remoteData?.captures?.length || remoteData?.suggestions?.length) {
          setData(remoteData);
        }
      })
  ...
  useEffect(() => {
    ...
    if (!isFirebaseConfigured || !user?.uid) return;
    saveUserData(user.uid, data).catch((error) => console.warn('Failed to sync to Firestore, local copy kept.', error));
  }, [data, user?.uid]);
  ...
  if (isFirebaseConfigured && user === undefined) {
    return <div className="stack"><p>Checking authentication…</p></div>;
  }

  if (isFirebaseConfigured && !user) {
    return <Layout><LoginGate /></Layout>;
  }
